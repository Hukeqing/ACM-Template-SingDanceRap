 ...
 */
const int MAXN = 60010;     // 点数
const int M = 2500010;      // 根据点数扩大，至少是 MAXN 的 40 倍
int n, q, m, tot;
int a[MAXN], t[MAXN];
...
int main()
{
    ...
    while (Tcase--)
    {
        ...
        for (int i = 0; i < q; i++)
        {
            ...
            {
                Modify(query[i].l, myhash(a[query[i].l]), -1);
                Modify(query[i].l, myhash(query[i].r), 1);
